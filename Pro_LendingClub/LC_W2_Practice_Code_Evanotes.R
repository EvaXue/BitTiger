...
# calculate the difference of interest rate between the two groups in term.
# 	Welch Two Sample t-test

#data:  int_rate by term
#t = -431.12, df = 467040, p-value < 2.2e-16
#alternative hypothesis: true difference in means is not equal to 0
#95 percent confidence interval: (mean difference M1-M2)
# -4.111525 -4.074310
#sample estimates:
...
##compare the grade, separate by term 
...
#if some features we do not have the reason to explain why it has so many NA, 
#we can remove them. 
...
# by => apply function to each subset
...
test <- loan[-train.ind, ]
# need to do missing value imputation. any feature with missing value, the observation will be removed.
